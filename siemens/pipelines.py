...
class SiemensPipeline:

    # Database setup
    conn = sqlite3.connect('siemens.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `siemens`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM siemens WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO siemens (date, title, link, content)"
